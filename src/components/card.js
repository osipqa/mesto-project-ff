...
const updatedCards = cards.map(card => {
  const randomUser = users[Math.floor(Math.random() * users.length)]; // Выбираем случайного пользователя
  return {
    ...card,
    owner: {
...
function handleLike(card, userID, cardElement) {
  const buttonLike = cardElement.querySelector('.card__like-button');
  const buttonCount = cardElement.querySelector('.card__like-count');
  const likeAction = check(card, userID) ? deleteLike : addLike; // Выбираем действие в зависимости от наличия лайка
...
updatedCards.forEach(card => handleLike(card, card.owner._id, cardElement));
*/
